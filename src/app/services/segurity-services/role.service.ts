...
import {
  ...
  IRoleMenuResponse,
  IRoleResponse,
} from '../../interfaces/segurity-interfaces/role.interface';
...
export class RoleService {
  ...
  onGetMenus() {
    return this._http
      .get<IRoleMenuResponse>(admin_service + '/menu/create-role/all')
      .pipe(
        map((response) => {
          ...
          const menusFinal = pathers.map((e) => {
            const childrens = children
              ...
              .map((a) => {
                ...
                return {
                  ...rest,
                  actions: actions.map((rec) => {
                    return { selected: false, action: rec };
                  }),
                };
              });
            ...
            e.actions =
              e.actions?.map((a) => {
                return { selected: false, action: a };
              }) ?? [];
