...
} from 'src/app/interfaces/admin-interfaces/customer.interface';
...
export class CustomerService {
  ...
  onFindAll(filter: IPagerFilter, page: number) {
    ...
    return this._http.get<ICustomerListResponse>(
  ...
  onFindById(id: string) {
    return this._http
      .get<ICustomerByIdResponse>(admin_service + `${entity}/${id}`)
      .pipe(
        map((response) => {
          const newResponse = { ...response };

          newResponse.data.departments = newResponse.data.departments.filter(
            (e) => e.status
          );
          newResponse.data.rates = newResponse.data.rates.filter(
            (e) => e.status
          );
          newResponse.data.activeNumbers =
            newResponse.data.activeNumbers.filter((e) => e.status);
