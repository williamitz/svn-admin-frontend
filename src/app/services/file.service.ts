...
export class FileService {
  ...
  uploadInterpreter( body: any, id?: string ) {
    return this._http.post( admin_service + `/file-user${ id ? `/${id}` : '' }`, body );
  }
  ...
  findAll( page: number, limit: number, id?: string ) {
    ...
    return this._http.get<IFileInterpreterListResponse>( admin_service + `/file-user${ id ? `/findById/${id}` : '' }?${params}` );
  }
